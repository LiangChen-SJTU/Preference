...
const { assignTimeSlots, TIME_SLOTS, MAX_TIME_USERS } = require('./time');
...
function validateSubmission(name, preferences) {
  ...
  if (!Array.isArray(preferences) || preferences.length !== TIME_SLOTS.length) {
    return `请选择全部 ${TIME_SLOTS.length} 个时段志愿`;
  }
