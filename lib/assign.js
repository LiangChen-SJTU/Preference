...
function assignTopics(submissions, options = {}) {
  ...
  const specificCapacities = balancedCapacities(specificSubs.length, REGULAR_TOPICS.length);

  const specificResults =
    specificSubs.length > 0 ? runRegularFlow(specificSubs, specificCapacities, rng) : [];
