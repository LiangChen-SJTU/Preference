...
/**
 * 对常规志愿用户做最小费用最大流分配。
 * 各课题容量按人数均分（硬约束），费用 = 100^(志愿顺位-1) 体现志愿优先（软约束）。
 */
function assignRegularTopics(submissions, options = {}) {
  ...
  const capacities = balancedCapacities(n, m);
  ...
  return { results: submissions.map((s) => resultByName[s.name]), tieBreakSeed };
}
...
function assignTopics(submissions, options = {}) {
  ...
  const regularSubs = submissions.filter((s) => !isOtherChoice(s.preferences));
  ...
  const regularAssignment =
    regularSubs.length > 0
      ? assignRegularTopics(regularSubs, options)
      : { results: [], tieBreakSeed: 0 };

  const resultByName = Object.fromEntries(
    [...otherResults, ...regularAssignment.results].map((r) => [r.name, r])
  );
  ...
  return {
    ...
    topicStats,
    summary,
    tieBreakSeed: regularAssignment.tieBreakSeed,
