const API = '/api/task-allocation';

let config = { tasks: [], anyChoice: '随便', maxUsers: 14 };
...
function renderAssignment(assignment) {
  ...
  els.groupStats.innerHTML = `
    <div class="topic-stat-grid">
      ${assignment.groupStats.map((g) => `
        <div class="topic-stat-item">
          <div class="topic-name">${escapeHtml(g.task)}</div>
          <div class="members">${escapeHtml(g.members.join('、') || '—')}</div>
        </div>
      `).join('')}
    </div>`;
}
